...
export default function Experience() {
  ...
  return (
    <section id="experience" className="py-24 px-6 relative bg-[#050812]">
      <div className="max-w-6xl mx-auto">
        ...
        <div className="relative px-2 sm:px-0">
          {/* Timeline Line */}
          <div className="absolute left-4 md:left-1/2 top-0 bottom-0 w-px bg-gradient-to-b from-blue-600/50 via-blue-500/20 to-transparent md:-translate-x-1/2" />
          ...
          <div className="space-y-8 md:space-y-12">
            {experiences.map((exp, index) => (
              ...
              >
                {/* Timeline dot */}
                <div className="absolute left-[13px] md:left-1/2 top-8 md:top-0 w-3 h-3 bg-blue-600 rounded-full md:-translate-x-1/2 z-20 shadow-[0_0_15px_rgba(59,130,246,0.5)]" />
                ...
                <div className="flex-1 pl-10 md:pl-0">
                  <div className={`glass rounded-3xl p-6 md:p-8 relative overflow-hidden group hover:bg-white/5 transition-colors duration-500 ${index % 2 === 1 ? 'text-left md:text-right' : 'text-left'}`}>
                    ...
                    <p className="text-base md:text-lg font-medium text-slate-300 mb-4">{exp.company}</p>
                    ...
                    <div className={`flex flex-wrap gap-2 ${index % 2 === 1 ? 'md:justify-end' : ''}`}>
                      {exp.highlights.map((skill, i) => (
                        <span
                          key={i}
                          className="px-2 md:px-3 py-1 bg-white/5 text-slate-300 text-[10px] md:text-xs rounded-full border border-white/10"
                        >
                          {skill}
                        </span>
                      ))}
