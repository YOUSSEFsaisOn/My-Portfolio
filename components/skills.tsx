      ...
      <div className="max-w-7xl mx-auto">
        ...
        >
          <h2 className="text-[clamp(32px,5vw,64px)] font-black text-[#1A103C] leading-tight font-serif tracking-tighter">
            TECHNICAL <br />
            <span className="text-[#3b82f6] drop-shadow-[0_0_15px_rgba(59,130,246,0.15)]">EXPERTISE.</span>
          </h2>
        </motion.div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-x-8 gap-y-12">
          {skillCategories.map((cat, index) => (
            <motion.div
              ...
              className="space-y-6"
            >
              <h3 className="text-xs font-black uppercase tracking-[0.2em] text-[#1A103C] border-b border-[#E1EEF8] pb-4 font-sans">
                {cat.title}
              </h3>
              <ul className="space-y-3">
                {cat.skills.map((skill, i) => (
                  <li key={i} className="text-sm font-bold text-slate-600 hover:text-[#1A103C] transition-colors cursor-default font-sans">
                    {skill}
                  </li>
                ))}
              </ul>
